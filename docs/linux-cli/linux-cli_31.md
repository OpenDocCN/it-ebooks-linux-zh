...
这样，如果 cd 命令运行失败后，rm 命令将不会执行。这样比较好，但是仍然有可能未设置变量 dir_name 或其变量值为空，从而导致删除了用户主目录下面的所有文件。这个问题也能够避免，通过检验变量 dir_name 中包含的目录名是否真正地存在：
...
*   Wikipedia 上面有两篇关于语法和逻辑错误的短文：
    ...
    [`en.wikipedia.org/wiki/Syntax_error`](http://en.wikipedia.org/wiki/Syntax_error)

    [`en.wikipedia.org/wiki/logic_error`](http://en.wikipedia.org/wiki/logic_error)

*   网上有很多关于技术层面的 bash 编程的资源：

    [`mywiki.wooledge.org/BashPitfalls`](http://mywiki.wooledge.org/BashPitfalls)

    [`tldp.org/LDP/abs/html/gotchas.html`](http://tldp.org/LDP/abs/html/gotchas.html)

    [`www.gnu.org/software/bash/manual/html_node/Reserved-Word-Index.html`](http://www.gnu.org/software/bash/manual/html_node/Reserved-Word-Index.html)

*   想要学习从编写良好的 Unix 程序中得知的基本概念，可以参考 Eric Raymond 的《Unix 编程的艺术》这本 伟大的著作。书中的许多想法都能适用于 shell 脚本：

    [`www.faqs.org/docs/artu/`](http://www.faqs.org/docs/artu/)

    [`www.faqs.org/docs/artu/ch01s06.html`](http://www.faqs.org/docs/artu/ch01s06.html)

*   对于真正的高强度的调试，参考这个 Bash Debugger：

    [`bashdb.sourceforge.net/`](http://bashdb.sourceforge.net/)